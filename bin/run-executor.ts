...
if (process.env.NEOAI_TERMINAL_OUTPUT_PATH) {
  setUpOutputWatching(
    process.env.NEOAI_TERMINAL_CAPTURE_STDERR === 'true',
    process.env.NEOAI_STREAM_OUTPUT === 'true'
  );
}

if (!process.env.NEOAI_WORKSPACE_ROOT) {
  console.error('Invalid Neoai command invocation');
  process.exit(1);
}
...
function setUpOutputWatching(captureStderr: boolean, streamOutput: boolean) {
  ...
  const outputPath = process.env.NEOAI_TERMINAL_OUTPUT_PATH;
  ...
  async (message: {
    ...
    try {
      const statusCode = await run(
        process.cwd(),
        process.env.NEOAI_WORKSPACE_ROOT,
